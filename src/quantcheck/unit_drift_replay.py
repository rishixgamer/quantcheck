"""Private, manifest-assisted exact replay for Unit Drift faults."""
...
from quantcheck.schemas import DatasetSnapshot, UnitDriftManifest
from quantcheck.unit_drift_manifest import validate_unit_drift_manifest

__all__ = ["UnitDriftReplayError", "manifest_assisted_exact_unit_drift_replay"]


class UnitDriftReplayError(ValueError):
    """Raised when replay inputs do not exactly match private manifest truth."""


def manifest_assisted_exact_unit_drift_replay(
    snapshot: DatasetSnapshot,
    manifest: UnitDriftManifest,
) -> DatasetSnapshot:
    """Restore exact originals, or return an already-restored clean input."""
    validate_unit_drift_manifest(manifest)
    if not dataset_snapshot_identity_matches(snapshot):
        raise UnitDriftReplayError("snapshot identity does not match its content")
    if (
        snapshot.dataset_name != manifest.dataset_name
        or snapshot.as_of_date != manifest.snapshot_as_of_date
    ):
        raise UnitDriftReplayError("snapshot and manifest describe different cases")
    ...
    if (
        ...
        raise UnitDriftReplayError("snapshot is neither the exact corrupted nor clean artifact")
    ...
    for entry in manifest.entries:
        ...
        if actual != entry.corrupted_record:
            raise UnitDriftReplayError("corrupted record does not match manifest truth")
        if entry.original_record.record_id in snapshot_by_id:
            raise UnitDriftReplayError("corrupted snapshot contains both original and replacement")
    ...
    if (
        ...
        raise UnitDriftReplayError("replay did not restore the exact clean snapshot")
    return repaired
