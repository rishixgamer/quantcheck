...
| --- | --- | --- | --- |
| `quantcheck-hero.svg` | Architecture and trust boundary | `docs/METHODOLOGY.md`, `QUANTCHECK_PUBLIC_EVIDENCE_LEDGER.md` A-07–A-10 | Method diagram; no performance claim. |
| `lookahead-timeline.svg` | First-screen hero and concrete point-in-time example | `reference/QUANT_DATA_CHAOS_LAB_CONCEPT.txt`, `docs/faults/LOOK_AHEAD.md` | Controlled illustration; not an observed SEC defect. |
...
| `failure-analysis.svg` | Retained false positives, misses, and structural no-target cases | `release_evidence/final/public/aggregate_report.json`, `evidence/design_partner_beta/validation_aggregate.json` | v0.1 and v0.2 are separate synthetic evidence sets; v0.2 is not customer validation. |
